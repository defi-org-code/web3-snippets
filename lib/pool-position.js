...
async function getSushiswapPoolData(holderInfo, poolAddress, farmNumber, web3) {
    ...
    if (isAddress(holderInfo)) {
        ...
        if (farmNumber !== '') {
            ...
            poolDataTxs.push(masterChef.methods.pendingSushi(farmNumber, holderInfo).call());
    ...
    return newPoolData(poolAddress, holderInfo, totalLp, holderLp, lpTokenDecimals, positionValues, farmNumber !== '' ? MasterChefAddress : '', indirectFranctionOfPool, sushiRewardTokenData);     
}
