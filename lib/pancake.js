...
import { pancakePairAbi } from "./pancake-pair-abi";
import { bep20Abi } from "./bep20-abi";
//import { getTokenData, uint256ToBigNumber } from "./bep20-helpers";
...
export async function pancakeLpPosition({ bscWeb3, fromBlock, toBlock, pair, lp }) {
  ...
  console.log(`got token datas ${tokenDatas[0]}`)
...
function uint256ToBigNumber(amount, tokenData) {
...
async function getPairTokensData(pairContract, web3) {
  ...
  const token0Contract = new web3.eth.Contract(bep20Abi, tokens[0]);
  const token1Contract = new web3.eth.Contract(bep20Abi, tokens[1]);
  ...
  const tokenRawDatas = await Promise.all(getTokenDataTxs);
  ...
  return [
    generateTokenData(tokens[0], tokenRawDatas[0], tokenRawDatas[1], tokenRawDatas[2], tokenPrices[0]),
    generateTokenData(tokens[1], tokenRawDatas[3], tokenRawDatas[4], tokenRawDatas[5], tokenPrices[1])
  ];
}

function generateTokenData(address, decimals, name, symbol, coingecko) {
  return {
    Address: address,
    Decimals: decimals,
    Name: name,
